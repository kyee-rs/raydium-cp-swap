...
impl ConstantProductCurve {
    ...
    pub fn swap_base_input_without_fees(
        ...
        let destinsation_amount_swapped = numerator.saturating_div(denominator);
    ...
    pub fn swap_base_output_without_fees(
        ...
        let numerator = swap_source_amount.saturating_mul(destinsation_amount);
        let denominator = swap_destination_amount
            .checked_sub(destinsation_amount)
            .unwrap();
        let (source_amount_swapped, _) =
            numerator.checked_ceil_div(denominator).unwrap_or_default();
